...
description: "For the complete documentation index, see [llms.txt](#gt-docs-preview-unavailable). Markdown versions of documentation pages are available by appending `.md` to the page URL."
---

The [Responses API](#gt-docs-preview-unavailable) is our new API primitive, an evolution of [Chat Completions](#gt-docs-preview-unavailable) which brings added simplicity and powerful agentic primitives to your integrations.
...
- Built-in tools like [web search](#gt-docs-preview-unavailable), [file search](#gt-docs-preview-unavailable), [computer use](#gt-docs-preview-unavailable), [code interpreter](#gt-docs-preview-unavailable), and [remote MCPs](#gt-docs-preview-unavailable).
...
- [Reasoning](#gt-docs-preview-unavailable) models have a richer experience in the Responses API with [improved tool usage](#gt-docs-preview-unavailable). Starting with GPT-5.4, Chat Completions does not support tool calling with `reasoning_effort` values other than `none`.
- Structured Outputs API shape is different. Instead of `response_format`, use `text.format` in Responses. Learn more in the [Structured Outputs](#gt-docs-preview-unavailable) guide.
- The function-calling API shape is different, both for the function config on the request, and function calls sent back in the response. See the full difference in the [function calling guide](#gt-docs-preview-unavailable).
...
- Set `store: false` in the [store field](#gt-docs-preview-unavailable).
...
the [function calling docs](#gt-docs-preview-unavailable) for more detail on how function calling works in Responses.
...
Function-calling streams can also emit events such as `response.function_call_arguments.delta` and `response.function_call_arguments.done`. See the [streaming Responses guide](#gt-docs-preview-unavailable) and [Responses streaming events reference](#gt-docs-preview-unavailable).

### 8. Upgrade to native tools

If your application has use cases that would benefit from OpenAI's native [tools](#gt-docs-preview-unavailable), you can update your tool calls to use OpenAI's tools out of the box.
...
Based on developer feedback from the [Assistants API](#gt-docs-preview-unavailable) beta, we've incorporated key improvements into the Responses API to make it more flexible, faster, and easier to use. The Responses API represents the future direction for building agents on OpenAI.

The Assistants API was officially sunset on August 26, 2026, and is no longer available. Follow the [migration guide](#gt-docs-preview-unavailable) to update your integration to the Responses API.
