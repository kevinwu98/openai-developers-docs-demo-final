...
The OpenAI API provides a consistent interface to state-of-the-art AI [models](#gt-docs-preview-unavailable) for text generation, natural language processing, computer vision, and more. Get started by creating an API Key and running your first API call. Discover how to generate text, analyze images, build agents, and more.
...
securely [access the API](#gt-docs-preview-unavailable). Store the key
...
A simple API request to the [Responses API](#gt-docs-preview-unavailable) would look like this:
...
A simple API request to [Responses API](#gt-docs-preview-unavailable) would look like this:
...
A first API request to the [Responses API](#gt-docs-preview-unavailable) would look like this:
...
Congrats on running a free test API request! Start building real applications with higher limits and use [our models](#gt-docs-preview-unavailable) to generate text, audio, images, videos and more.
...
Give the model access to external data and functions by attaching [tools](#gt-docs-preview-unavailable). Use built-in tools like web search or file search, or define your own for calling APIs, running code, or integrating with third-party systems.
...
Use server‑sent [streaming events](#gt-docs-preview-unavailable) to show results as they’re generated, or use the [Realtime API](#gt-docs-preview-unavailable) for interactive voice apps and apps with text, audio, and image inputs.
...
Use the OpenAI platform to build [agents](#gt-docs-preview-unavailable) capable of taking action—like [controlling computers](#gt-docs-preview-unavailable)—on behalf of your users. Use the [Agents SDK](#gt-docs-preview-unavailable) to create orchestration logic on your server.
